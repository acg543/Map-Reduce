...
int getInterData(char *key, int reducerID) {
  //TODO open message queue 
  key_t keys;
	if ((keys = ftok("test/T1/F1.txt", MQUEUE - 1)) == -1) {
  ...
  if (msgrcv(msgid, (void*) &msg, strlen(msg.msgText) + 1, reducerID, IPC_NOWAIT) == -1) {
		printf("ERROR: Recieving message in getInterData\n");
    exit(0);
	}
...
void shuffle(int nMappers, int nReducers) {
  ...
  //TODO open message queue
  int msgid;
  key_t key = ftok("test/T1/F1.txt", MQUEUE - 1);
  if (key == -1) {
  ...
  while (ackCounter < nReducers) {
    ...
    if (msgrcv(msgid, &msgReceiver, sizeof(msgReceiver.msgText), ACKTYPE, 0) == -1) {
      printf("ERROR: Recieving ACK message in shuffle");
      exit(0);
    }
